...
#=============================================================#
# 設定ファイルの読み込み
config_file_path = parent_dir + "/configs/base_settings.yaml"
#
with open(config_file_path, "r", encoding="utf-8") as file:
    config = (yaml.safe_load(file))["hybrid_a_star"]

XY_GRID_RESOLUTION  = float(config["xy_grid_resolution_m"])  # [m]
YAW_GRID_RESOLUTION = np.deg2rad(float(config["yaw_grid_resolution_deg"]))  # [rad]
MOTION_RESOLUTION   = float(config["path_interpolate_resolution_m"])  # [m] path interpolate resolution
N_STEER             = int(config["number_of_steer_command"])  # number of steer command

SB_COST           = float(config["switch_back_penalty_cost"])  # switch back penalty cost
BACK_COST         = float(config["backward_penalty_cost"])  # backward penalty cost
STEER_CHANGE_COST = float(config["steer_angle_change_penalty_cost"])  # steer angle change penalty cost
STEER_COST        = float(config["steer_change_penalty_cost"])  # steer angle change penalty cost
H_COST            = float(config["heuristic_cost"])  # Heuristic cost
